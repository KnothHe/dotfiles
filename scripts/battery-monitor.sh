...
# source: https://azer.bike/journal/low-battery-monitor-for-linux-i3
...
checkBatteryLevel() {
    if [ $battery_state == "Charging" ] || [ "${battery_level}" == "${previous_battery_level}" ]; then
        exit
    fi
...
checkBatteryStateChange() {
    if [ "$battery_state" == "Charging" ] && [ "$battery_state" != "${previous_battery_state}" ]; then
